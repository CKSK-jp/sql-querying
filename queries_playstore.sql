...
FROM
  analytics
WHERE
  min_installs <= 50 and rating IS NOT null
ORDER BY
  rating;
...
FROM
  analytics
WHERE
  last_updated = (SELECT MIN(last_updated) FROM analytics);
...
SELECT
  app_name, reviews, min_installs, min_installs / reviews as proportion
FROM
  analytics
WHERE
  min_installs >= 100000
